...
pub enum Action {
    ...
    RunCheck,
    /// SENTINEL-1 (2.2, CT-P6) — the continuity ledger dashboard (Ctrl+B Shift+I):
    ...
    EnterBlockSelect,
    /// `Ctrl+Z g`. Prompt for a line number and jump the cursor there (centred),
    /// for chasing a Typst compile-error / diagnostic line.
    #[serde(rename = "editor.goto_line")]
    GotoLine,
    /// `Ctrl+Z w`. Toggle soft-wrap display for the open buffer (session-only;
    /// the persisted default is `editor.wrap`).
    #[serde(rename = "editor.toggle_wrap")]
    ToggleSoftWrap,
    /// `Ctrl+Z t`. Strip trailing whitespace from every line of the open buffer,
    ...
    OpenJournal,
    /// 1.2.17+ T.6 — `Ctrl+B Shift+V`.  Voice picker
    /// modal for the Piper TTS backend.  Lists every
    /// catalog voice + already-downloaded voices not in
    /// the catalog (offline fallback), with on-the-fly
    /// filter, download-on-Enter (blocking for cycle 1
    /// — background download is a T.6.b polish), and
    ...
    /// grounds the check against the established world
    /// facts, so the model flags any claim that
    /// contradicts the world (snow in the tropics, a
    /// three-day ride that should be three weeks).  The
    /// mnemonic is `X` for fact e**X**amination.  Streams
    /// into the AI pane under the fact-analysis system
    /// prompt.
    ...
    SearchFacts,
    /// Ctrl+B Shift+J (1.2.21+) — jump to the next finding
    /// from the last fact-check (Ctrl+B Shift+X): moves the
    /// editor cursor to each flagged claim in the open
    /// paragraph in turn, status-bar showing the violated
    /// fact.  Mnemonic `J` for Jump.
    #[serde(rename = "ai.next_fact_finding")]
    NextFactFinding,
...
impl Action {
    ...
    pub fn description(&self) -> String {
        match self {
            ...
            Action::OpenCanon =>
                "Open the CANON ledger dashboard (reader hub → Canon): the story's development-history decisions (world-facts, character traits, plot points, reveals, setups) with their kind and commitment level, plus any commitment forks. ↑↓ scroll, Enter jumps to the decision's source paragraph, g grounds it on a decision you then pick, h shows its development history in the Thoughts pane, t toggles the grounds-graph view (foundations → what rests on them), Esc closes. Populated deterministically from authored tags on save and by `inkhaven canon harvest`. The CLI equivalent is `inkhaven canon` (`list` / `impact` / `why` / `graph` / `commit` / `forks`).".into(),
            Action::OpenBonds =>
            ...
            Action::GotoLine =>
                "Go to line (Ctrl+Z g) — prompt for a line number and jump the cursor there, centred in the viewport. Useful for chasing a Typst compile-error or diagnostic line (the gutter marks diagnostic lines with a red ●). Enter jumps, Esc cancels. Mnemonic: G for Goto.".into(),
            ...
            Action::OpenWorldOverview =>
                "Open the World overview (WORLD-4, Ctrl+B W) — a read-only summary of the project's world simulation: the `world.hjson` definition (name / seed / star / planet / moons), and a compact summary of every compiled layer — astronomy (year length in planet-days with the declared-vs-computed divergence flag, axial tilt, moons, dominant tide, calendar consistency), geology (plates / continents / ocean %), climate (mean temperature, precipitation, biomes), hydrology (rivers / lakes / watersheds), and demographics (population / settlements) — each marked ✓ once materialized into the World system book (press C to compile the whole world). `↑↓` scroll, `Esc` closes. The operations live on the CLI — `inkhaven realworld new / validate / compile [--materialize]`. Mnemonic: W for World. (Typewriter / focus mode moved to Ctrl+B Shift+W.)".into(),
            Action::OpenInnerSocratesOverview =>
                "Open the Inner Socrates overview (INNER_SOCRATES-1, Ctrl+B J) — examined authorship: the active Reader Persona, recent Socratic questions about your prose, and the intent ledger (deliberate authorial choices the interrogator respects). Sub-keys: `S` select persona, `N` new persona wizard, `L` view ledger, `F` fast-check the open paragraph, `E` engage the slow Socratic pass, `T` Inner Theologian, `R` Inner Rigor, `Y` Inner Stylist, `P` Inner Poet (scan the open verse paragraph's metre and rhyme against its declared `poem:` form → Output pane). `↑↓` scroll, `Esc` closes. Inner Socrates produces questions, never corrections. The CLI surface is `inkhaven inner-socrates check / ledger`. Mnemonic: J (I was taken by book-info).".into(),
            Action::OpenInnerEditorOverview =>
                "Open the Inner Editor overview (INNER_EDITOR-1, Ctrl+V O) — the second Inner-family member: a thoughtful editor that OBSERVES literary craft (richness, tautology, style, vocabulary, belief stance, praise, suggestions) as Praise/Note/Concern, never prescribing. Sub-keys: `E` engage the open paragraph (an LLM pass), `C` open an Editor conversation about it (also F9 → Editor scope), `A` toggle the ambient paragraph-pause auto-engage (opt-in; LLM cost), `F` jump to the findings in Output. `↑↓` scroll, `Esc` closes. The CLI surface is `inkhaven inner-editor engage / findings / config / usage`. Mnemonic: O for Observe (Ctrl+B E was taken).".into(),
            ...
            Action::NextFactFinding =>
                "Jump to the next fact-check finding (1.2.21+, Ctrl+B Shift+J). After a fact-check (Ctrl+B Shift+X) flags contradictions, this cycles through them: each press moves the editor cursor to the next flagged claim in the open paragraph (best-effort — the model's quoted phrase may be paraphrased) and shows the violated fact + explanation on the status bar. Wraps around. Findings are tied to the paragraph that was checked — switch paragraphs and it asks you to re-run the check. Mnemonic: J for Jump.".into(),
    ...
    pub fn defaults() -> Self {
        Self {
            ...
            meta_sub: vec![
                ...
                // SENTINEL-1 (CT-P6) — Ctrl+B Shift+I: the continuity ledger (I for
                ...
                // KEN-1 (KEN-P5) — Ctrl+B Shift+Z: the knowledge dashboard. Shift+Z
                // is the LAST free meta_sub chord.
                ...
                // 1.2.17+ T.6 — Ctrl+B Shift+V opens
                // the Piper TTS voice picker (V for
                // Voice).  Catalog + downloaded voices,
            ...
            bund_sub: vec![
                ...
                // A4 editor niceties (Ctrl+Z is the utility prefix; g/w/t free):
            ...
            view_sub: vec![
                ...
                // 1.3.12 DEEP-1 — Ctrl+V Shift+F runs the deep AI world
                // refresh in the background (F for reFresh).
                entry("Shift+f", Action::RunDeepRefresh, Scope::Any),
...
mod tests {
    ...
    #[test]
    fn continuity_ledger_is_bound_to_ctrl_b_shift_i() {
        let k = KeyBindings::defaults();
        let ev = KeyEvent::new(KeyCode::Char('I'), KeyModifiers::SHIFT);
        assert_eq!(
            k.resolve_meta_sub(&ev, Focus::Editor),
            Some(Action::OpenContinuityLedger),
            "Ctrl+B Shift+I opens the continuity ledger"
        );
    }

    #[test]
    fn new_dashboards_do_not_shadow_existing_ctrl_b_chords() {
        // The whole-book dashboards (continuity ledger, read-through) must not
        // shadow an existing Ctrl+B chord at any scope — the Shift+L→concordance
        // and Shift+S→Search-Facts lessons.
        let k = KeyBindings::defaults();
        let sub = |c: char| {
            k.resolve_meta_sub(&KeyEvent::new(KeyCode::Char(c), KeyModifiers::SHIFT), Focus::Editor)
        };
        ...
        assert_eq!(sub('Z'), Some(Action::OpenKnowledge));
        // The shifted dashboards must not shadow the bare Ctrl+B letters (the
        // matcher tracks Shift separately): u = UndoLastDelete, z = OpenGraphHub.
        let plain = |c: char| {
            k.resolve_meta_sub(&KeyEvent::new(KeyCode::Char(c), KeyModifiers::NONE), Focus::Editor)
        };
        assert_eq!(plain('u'), Some(Action::UndoLastDelete), "bare Ctrl+B u still UndoLastDelete");
        assert_eq!(plain('z'), Some(Action::OpenGraphHub), "bare Ctrl+B z still the graph hub");
    }

    #[test]
    fn knowledge_is_bound_to_ctrl_b_shift_z() {
        let k = KeyBindings::defaults();
    ...
    #[test]
    fn a4_editor_niceties_are_bound_on_ctrl_z() {
        // A4 — go-to-line / soft-wrap toggle / strip-trailing-whitespace live on
        // the Ctrl+Z utility layer (g/w/t), Editor-scoped, plain letters (no
        // terminal-ambiguous Shift).
        let k = KeyBindings::defaults();
        ...
        for (ch, action) in [
            ...
            assert_eq!(
                k.resolve_bund_sub(&ev, Focus::Editor),
                Some(action),
                "Ctrl+Z {ch} should be bound in the editor"
            );
        }
    }
    ...
    #[test]
    fn cost_dashboard_is_bound_to_ctrl_b_dollar() {
        let k = KeyBindings::defaults();
        let ev = KeyEvent::new(KeyCode::Char('$'), KeyModifiers::NONE);
        assert_eq!(
            k.resolve_meta_sub(&ev, Focus::Editor),
            Some(Action::OpenCostDashboard),
            "Ctrl+B $ opens the AI cost dashboard"
        );
    }
