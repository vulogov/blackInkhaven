...
fn global_entries() -> Vec<Entry> {
    vec![
        ...
        entry("Ctrl+1/3/4/5", "Focus Editor/AI/Search/AI-prompt"),
        entry("Ctrl+2 · Ctrl+B Shift+O", "Open the full-screen Outline pane"),
        entry("Ctrl+T", "Focus the side Tree pane"),
...
fn tree_entries() -> Vec<Entry> {
    vec![
        ...
        blank(),
        header("─ Copy / move paragraph (shared with Outline) ─"),
        entry("y / m", "Copy / move cursor paragraph onto the clipboard"),
        entry("f", "Affix it into/after the cursor node"),
        blank(),
        header("─ Folding ─"),
