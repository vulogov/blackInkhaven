...
impl super::super::App {
    ...
    /// CHRONICLE-1 (CH-P4) — the draft-history dashboard: the trend since the last
    /// milestone + the cleared/introduced split. Same scrollable-rows shape as the
    /// continuity ledger; introduced-finding rows carry a jump anchor.
    pub(in crate::tui::app) fn draw_chronicle_modal(&self, f: &mut ratatui::Frame, area: Rect) {
        let Modal::Chronicle { rows, anchors, cursor } = &self.modal else {
            return;
        };
        let width = area.width.saturating_sub(6).clamp(52, 92);
        let height = area.height.saturating_sub(4).max(12);
    ...
    /// BONDS-1 (BD-P4) — the relationship dashboard. KEN's sibling; identical
    /// layout to [`draw_knowledge_modal`], different title + axis.
    pub(in crate::tui::app) fn draw_bonds_modal(&self, f: &mut ratatui::Frame, area: Rect) {
        let Modal::Bonds { rows, anchors, cursor } = &self.modal else {
            return;
        };
        let width = area.width.saturating_sub(6).clamp(52, 92);
        let height = area.height.saturating_sub(4).max(12);
