...
mod tests {
    ...
    fn build_root_typ_bibliography_line_is_style_gated() {
        ...
        let with = build_root_typ(&book, "", Some("ieee"), "", false);
        assert!(
        ...
        let without = build_root_typ(&book, "", None, "", false);
        assert!(!without.contains("#bibliography"), "{without}");

        // With the Index Locorum enabled, its include follows the bibliography.
        let loci = build_root_typ(&book, "", Some("ieee"), "", true);
        let bib_i = loci.find("#bibliography").unwrap();
        let inc = loci.find("#include \"index_locorum.typ\"").expect("loci include");
        assert!(inc > bib_i, "index locorum must follow the bibliography");
    }
