...
fn build_root_typ(
    ...
    // Single-line form of the title for the `//` comment lines — a newline in a
    // title (rejected by the TUI, but defensively) would otherwise break out of
    // the comment into active code.
    let title_line = book.title.replace(['\n', '\r'], " ");
    let mut out = String::new();
    out.push_str("// Auto-generated by inkhaven Book assembly.\n");
    out.push_str(&format!("// Book: {title_line}\n\n"));
...
/// Backslash-escape the Typst markup metacharacters so a plain string can sit in
/// *content* (markup) position — a heading body, front matter, an index label —
/// without a stray `$`/`#`/`@`/… breaking the compile or injecting markup. The
/// canonical content-mode escaper (front matter and the paragraph seed heading
/// both use it).
...
mod tests {
    ...
    #[test]
    fn escape_typst_content_escapes_markup_metachars() {
        // 3.0.9 — a seed heading / front matter from a special-char title must be
        // valid Typst content (no unmatched `$` math, no `#`/`@` injection).
        assert_eq!(escape_typst_content("Cost $5"), "Cost \\$5");
        assert_eq!(escape_typst_content("The #1 Rule"), "The \\#1 Rule");
        assert_eq!(escape_typst_content("A [draft] @ref"), "A \\[draft\\] \\@ref");
        // Ordinary + non-ASCII text passes through untouched.
        assert_eq!(escape_typst_content("plain title"), "plain title");
        assert_eq!(escape_typst_content("Москва"), "Москва");
    }
