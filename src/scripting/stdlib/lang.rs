...
fn do_translate(vm: &mut VM) -> Result<&mut VM> {
    ...
    langapi::emit_translation_output(&name, &text, &t.target, t.confidence, "forward", &t.unresolved);
    let gloss =
...
fn do_reverse(vm: &mut VM) -> Result<&mut VM> {
    ...
    langapi::emit_translation_output(&name, &surface, &r.english, r.confidence, "reverse", &r.unresolved);
...
fn do_cross(vm: &mut VM) -> Result<&mut VM> {
    ...
    langapi::emit_translation_output(&from, &surface, &c.target, c.confidence, "cross", &c.unresolved);
