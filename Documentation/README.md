...
| [`GRAPH.md`](GRAPH.md)                 | The semantic net (2.0, RFC SEMNET-1) — a typed-edge knowledge-graph layer over the nodes you already have: the edge model (kinds / endpoints / origin & durability), how to populate it (`graph rebuild` structural edges, `graph lexical` WordNet bridge, live confront stance), the `inkhaven graph` command (`stats` / `neighbors` / `contradicting` / `loci` / `paths` / `promote` / `dismiss`), multilingual behaviour, and `edges.db` crash-safety. |
