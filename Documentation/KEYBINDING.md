...
| `Ctrl+V q` | (1.3.1) **Submission-package generator** — pick a piece (query letter / synopsis / comps / logline) and stream it into the AI pane from the current book's **cached** digest (build it first with `inkhaven submission digest`). The system prompt resolves through the three tiers (Prompts-book paragraph `submission-*` → `prompts.hjson` → built-in, language-aware), so you can tune the house voice. `↑↓` pick, `Enter` → AI pane, `Esc` cancels. Mnemonic: **Q** for **Q**uery. | `view.open_submission_generator` |
| `Ctrl+V Shift+K` | (1.3.2) **Structure outline** — the `inkhaven plan check` report for the current book as a position bar per beat: `|` is the framework's target position, `●` where the beat actually lands (`#` when they coincide), colour-coded on-target / drift / gap, with act word-share pacing and an `↪N` thread chip. Scaffold a framework first with `inkhaven plan init`; map a beat by setting `mapped_chapter` in its Planning-book paragraph. `↑↓` navigate, **`a`** streams the AI structure analysis into the AI pane (over the book's cached digest), `Esc` closes. Mnemonic: **K** for s**K**eleton. | `view.open_plan_outline` |
