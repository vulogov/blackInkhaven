...
| `Ctrl+B \|`          | (1.2.8) **Edit project HJSON** — open `<project>/inkhaven.hjson` in a full-screen modal editor with HJSON syntax highlighting. `Ctrl+S` saves; when the saved bytes differ from the loaded bytes, a *Restart required* overlay pops up (config applies on next launch). `Esc` closes (status-line warning fires if there are unsaved edits). All standard `tui-textarea` editing chords work: arrows, Home/End, Shift+arrows for selection, Ctrl+Z undo, Ctrl+Y redo. | `bund.edit_project_hjson` |
