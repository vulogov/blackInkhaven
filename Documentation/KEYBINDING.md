...
| `Ctrl+V g` (editor) | (2.0, SEMNET) **Graph neighbourhood** — open a read-only tree of the open paragraph's one-hop knowledge-graph edges (what it links to, contradicts, is sourced from, cites, and the word-senses it mentions), grouped by kind with direction arrows (`→` out, `←` in, `⇄` symmetric). `↑↓` scroll, `Esc` closes. Populate the graph with `inkhaven graph rebuild` / `graph lexical`; stance edges accrue via `Ctrl+V ?` confront. On a **confront finding** in the Output pane, **`P`** promotes its stance edge (kept across `graph rebuild`) and **`d`** rejects it (deletes the edge). See `Documentation/GRAPH.md`. Mnemonic: **g** for **g**raph. | `graph.open_neighbourhood` |
...
| `Ctrl+V Shift+R` | (1.3.6) **The Editorial Pass** — one ranked revision worklist unifying every detector (the editorial `doctor` classes + `plan check`'s structural findings + the Facts-scan sidecar + the prose-style detectors: show-don't-tell, filter-word, anachronism), errors first; the same as `inkhaven edit`, deterministic (reads computed sidecars, no live AI). `↑↓` navigate (the selected finding's message + hint expand below), `[`/`]` cycle the category filter, **`Enter`** jumps to the finding's chapter/paragraph in the editor (a `→` marks the jumpable ones), **`f`** streams an AI rewrite of a rewritable finding marked `✎` (echo / pacing rewrite the whole paragraph; show-don't-tell / filter-word rewrite just the flagged phrase — 1.3.9 span-scoped) and pops the diff-review (`a` accept snapshot-first, `r` reject), **`F`** (1.3.9) batch fix-all — walks every rewritable finding in the current filter through that same review (`Esc` in the diff stops it), **`s`** skips it for the session, **`d`** defers it (persisted — hidden until the prose changes), **`D`** clears all deferrals, `Esc` closes. `edit` is *manuscript readiness*; `doctor` (`Ctrl+B Shift+0`) stays *project integrity*. Mnemonic: **R** for **R**evision pass. | `view.open_editorial_pass` |
