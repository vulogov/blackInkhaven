...
| `W` / `w`              | any (needs current user book) | (1.2.5) **Story view** — build a DOT graph of the current book's hierarchy (chapters / subchapters / paragraphs / scripts / images / json), wiki-links (dashed purple), and Characters / Places / Artefacts mentions (dashed green); lay it out via `layout-rs`, rasterise via `resvg`, float the PNG on top of the editor. Inside the modal: `Esc` closes, `S` opens a save-as picker (default `<book-slug>-story-YYYYDDMM-HHMM.png`). |
