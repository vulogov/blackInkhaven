...
| `Ctrl+V Shift+L` | (1.3.8) **Story bible** — a consolidated, navigable view of the world: every Character with the attributes `inkhaven continuity extract` has tracked across chapters beneath it (`eye_color: brown (ch.3)`), plus the Places, Artefacts, and Facts books. `↑↓` navigate, **`Enter`** jumps to an entry's source paragraph (a `→` marks the jumpable rows), `Esc` closes. Mnemonic: **L** for **L**ore. | `view.open_story_bible` |
