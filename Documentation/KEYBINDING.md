...
| `Ctrl+V Shift+K` | (1.3.2) **Structure outline** — the `inkhaven plan check` report for the current book as a position bar per beat: `|` is the framework's target position, `●` where the beat actually lands (`#` when they coincide), colour-coded on-target / drift / gap, with act word-share pacing, an `↪N` thread chip, and (1.3.4) a **tension overlay** — expected vs actual intensity sparklines beneath the bars, flagging beats that land flat. Scaffold a framework first with `inkhaven plan init`. The view is interactive: `↑↓` navigate, **`m`** maps the selected beat to a chapter from a picker (write-back, no HJSON editing), **`t`** links threads (Space toggles), **`s`** cycles its status, **`Enter`** opens the beat's mapped chapter in the editor, **`v`** flips to the scene board (cards grouped by chapter, goal/conflict/disaster spine), **`a`** streams the AI structure analysis into the AI pane (over the book's cached digest) — press **`L`** there to file the analysis as a *Structural Analysis* paragraph in the **Planning** book — `Esc` closes. Mnemonic: **K** for s**K**eleton. | `view.open_plan_outline` |
