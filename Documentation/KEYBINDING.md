...
| `Ctrl+B z` | (2.0/2.x, SEMNET + GRAPHMIND) **Graph hub** — a small menu onto the knowledge graph: **`n`** opens a read-only tree of the open paragraph's one-hop edges (what it links to, contradicts, is sourced from, cites, the senses it mentions), grouped by kind with direction arrows (`→` out, `←` in, `⇄` symmetric), `↑↓` scroll; **`i`** opens the **edge inbox** — the advisory `judged` stance edges awaiting triage (from confront, `graph link`, deep research), where **`P`** promotes the selected edge (kept across `graph rebuild`) and **`d`** rejects it; **`w`** (GM-P8) **walks the graph** to answer the question typed in the AI prompt — the AI pane streams the exploration (search → neighbours → contradictions → paths) then the grounded prose answer, `Esc` stops the walk, status shows `turn k/N`. `Esc` closes. Populate the graph with `inkhaven graph rebuild` / `graph lexical` / `graph link`. On a **confront finding** in the Output pane, **`P`**/**`d`** also promote/reject its edge. See `Documentation/GRAPH.md`. The graph's meta hub, alongside `Ctrl+B W`/`J`/`X`. | `graph.open_hub` |
