...
| -------- | ---------- | ------------------------------------ | ---------- |
| **1.2.8** | _in flight_ | [`1.2.8.md`](1.2.8.md)              | **Picking up what 1.2.7 left on the table.** Planned themes: `inkhaven export timeline` (SVG/PNG/Typst); cascade-impact AI on event re-time (opt-in via `Ctrl+V Shift+I`); Scrivener `<CustomMeta>` date-import bridge; F11-on-paragraph "suggest event" variant; persisting the `Alt+←/→` navigation ring across restarts; `editor.mouse_captured` HJSON knob.  Living scratchpad — see [`1.2.8.md`](1.2.8.md) for the current state. |
