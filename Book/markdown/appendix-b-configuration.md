...
Every HJSON knob, by stanza. The canonical machine-readable reference lives at `Documentation/CONFIGURATION.md`; this appendix is the condensed printed companion.
...
API keys come from environment variables: `GEMINI_API_KEY`, `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `DEEPSEEK_API_KEY`, `XAI_API_KEY`. Ollama needs none.
...
## typst_page / typst_fonts / typst_layout
...
False when the host terminal can't render images — kills the half-block fallback that's usually worse than no preview.
...
Custom calendar block — see Chapter 17 + 5 for the full shape.
...
Categories: `store_read`, `store_write`, `fs_read`, `fs_write`, `net`, `shell`, `code_eval`, `keymap`, `ai_write`, `editor_write`, `theme_write`. Default deny: `fs_write`, `net`, `shell`, `code_eval`, `keymap`.
...
The full list of theme keys is long; see `Documentation/CONFIGURATION.md`. Most-used:
