...
#grid(
  ...
  [
    ...
    for writers carries the same instinct into the making of worlds.
  ],
)
...
#section("Why the World Simulation exists")

Inkhaven is Vladimir's personal reflection on how a literary tool can help the
people who write books. The World Simulation is one answer to a specific
frustration: that the world a writer invents lives in scattered notes, a map in
one program and a wiki in another, and a binder that quietly begins to contradict
itself somewhere around chapter twelve — and that none of it is anywhere near the
page where the story is actually written.

A novelist keeps a map open in one window, a timeline in a spreadsheet, and a
folder of half-remembered decisions about how magic works; by the third act the
sun rises in the wrong place and a rider crosses a continent in an afternoon. The
World Simulation was built to close that gap: to make the world a *room inside the
writing tool*, where the physics is consistent because it is computed rather than
remembered, where the same seed always grows the same world, and where that world
flows straight back into the manuscript — as places, as a calendar, as the weather
of the very scene under your cursor.
...
This was a deliberate choice. There are excellent commercial tools for
worldbuilding and writing. They cost money — which is fine for many writers, and
a barrier for many more. Inkhaven exists for the second group: for the novelist
building a trilogy on a battered laptop, for the game-master who shouldn't have to
pick between rent and software, for the engineer drafting in the same terminal
where they already write code, for anyone who would benefit from a tool that
respects their work without asking for a credit-card number.
