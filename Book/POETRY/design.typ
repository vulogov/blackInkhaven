// Poetry with Inkhaven — design tokens + page chrome.
//
// Shares the voice and chrome of the companion books (Developing a Constructed
// Language; Linguistic Research with Inkhaven), self-contained and built on
// Typst-bundled fonts so it compiles warning-free with a bare `typst compile`.

#let book_title    = "Poetry with Inkhaven"
#let book_subtitle = "Reading, Measuring, and Making Verse"
...
#let book(pages) = {
  ...
  block(width: 100%, height: 100%)[
    ...
    #place(top + center, dy: 64mm, block(width: 70%)[
      ...
      #align(center)[
        #text(font: body_family, size: 12pt, tracking: 4pt, fill: ink_smoke, upper("For the Poet and the Critic"))
        #v(11mm)
    ...
    #place(bottom + center, dy: -30mm, align(center)[
      ...
      #v(2mm)
      #text(font: body_family, size: 9pt, fill: ink_smoke, book_year + " · examples assume Inkhaven 1.8.17 or newer")
    ])
  ]
  pagebreak()
