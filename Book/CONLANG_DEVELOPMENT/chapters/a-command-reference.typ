...
/ `frequency <lang> [--lemma] [--top N] [--json]`: A word-frequency list and descriptive statistics (tokens, types, type–token ratio, the Zipf fit) over the stored texts. `--lemma` counts a root's inflected forms together.
/ `concordance <lang> --word W [--lemma] [--window N] [--json]`: A KWIC concordance — every occurrence of a word across the stored texts, with its context. `--lemma` matches a root's inflected forms too.
/ `collocations <lang> --word W [--lemma] [--window N] [--top N] [--json]`: The collocates of a word — the neighbours within its window, ranked by co-occurrence and scored by PMI (a distinctive collocate outranks a merely-frequent word).
