...
pub fn register(module: &mut RpcModule<()>) {
    module
        .register_async_method("v2/doc.get", |params, _ctx, _| async move {
            log::debug!("v2/doc.get: start");
            let p: DocGetParams = params.parse()?;
            let id = uuid::Uuid::parse_str(&p.id)
                .map_err(|e| rpc_err(-32600, format!("invalid UUID {:?}: {e}", p.id)))?;
            let id_str = p.id.clone();
            let result = tokio::task::spawn_blocking(move || {
                log::debug!("v2/doc.get: session={} id={}", p.session, id);
