...
struct PrimariesExploreTelemetryParams {
    #[allow(dead_code)]
    session: String,
    duration: String,
}
    ...
    module
        .register_async_method(
            ...
            |params, _ctx, _| async move {
                log::debug!("v2/primaries.explore.telemetry: start");
                let p: PrimariesExploreTelemetryParams = params.parse()?;

                let result = tokio::task::spawn_blocking(move || {
                    log::debug!(
                        "v2/primaries.explore.telemetry: session={} duration={}",
                    ...
                    let entries = db
                        .primaries_explore_telemetry(&p.duration)
                ...
                log::debug!("v2/primaries.explore.telemetry: done");
                result
            },
        )
        .unwrap();
