...
struct SummaryForQueryParams {
    /// Caller transaction UUIDv7.  Accepted for symmetry with other v2
    /// methods; not consulted internally.
    #[allow(dead_code)]
    session: String,

    /// Plain-text vector query — wrapped server-side as a JSON string and
    /// embedded with the shared model.
    query: String,
...
pub fn register(module: &mut RpcModule<()>) {
    module
        .register_async_method("v2/summary_for_query", |params, _ctx, _| async move {
            log::debug!("v2/summary_for_query: start");
            let p: SummaryForQueryParams = params.parse()?;

            let result = tokio::task::spawn_blocking(move || {
                log::debug!(
                    "v2/summary_for_query: query={:?} max_sentences={} ratio={}",
                ...
                let summary = db
                    .summary_for_query(txn_id, &p.query, &cfg)
            ...
            log::debug!("v2/summary_for_query: done");
            result
        })
        .unwrap();
}
