#set document(title: "BUND Basic Library Reference")
#set page(
  paper: "a4",
...
#align(center)[
  #text(size: 20pt, weight: "bold")[BUND Basic Library Reference]
  #v(0.3cm)
  #text(size: 12pt)[Complete word (built-in command) reference]
  #v(0.5cm)
...
```bund
true println                  // true
42 "hello" pair println       // (42, "hello")
...
#table(
  ...
  fill: (col, row) => if row == 0 { luma(230) } else { white },
  [*Word*], [*Stack effect*], [*Description*],
  [`==`], [`( a b -- bool )`], [`a` equal to `b`.],
  [`!=`], [`( a b -- bool )`], [`a` not equal to `b`.],
...
== `if.` — workbench variant

```
( lambda W:bool -- )
```

The condition is read from the workbench.
...
== `notifthenelse.` — workbench variant
...
== `times.` — workbench variant

```
( lambda W:n -- )
```

= Stack and Value Manipulation
...
#table(
  ...
  [`format`],            [`( data... template -- str )`], [Template substitution using `{key}` placeholders.],
...
= Lambda and Function Management
...
Variables provide named, persistent storage outside of any stack.
